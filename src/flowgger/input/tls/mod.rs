...
fn get_default_threads(config: &Config) -> usize {
    config.lookup("input.tls_threads").map_or(DEFAULT_THREADS, |x| {
        x.as_integer().expect("input.tls_threads must be an unsigned integer") as usize
    })
}
...
pub fn config_parse(config: &Config) -> (TlsConfig, String, u64) {
    ...
    let cert = config.lookup("input.tls_cert")
        .map_or(DEFAULT_CERT,
                |x| x.as_str().expect("input.tls_cert must be a path to a .pem file"))
        .to_owned();
    let key = config.lookup("input.tls_key")
        .map_or(DEFAULT_KEY,
                |x| x.as_str().expect("input.tls_key must be a path to a .pem file"))
        .to_owned();
    let ciphers = config.lookup("input.tls_ciphers")
    ...
    let verify_peer = config.lookup("input.tls_verify_peer").map_or(DEFAULT_VERIFY_PEER, |x| {
        x.as_bool().expect("input.tls_verify_peer must be a boolean")
    });
    ...
    let compression = config.lookup("input.tls_compression").map_or(DEFAULT_COMPRESSION, |x| {
        x.as_bool().expect("input.tls_compression must be a boolean")
    });
    ...
    let framing = config.lookup("input.framing")
        .map_or(framing, |x| {
            x.as_str().expect(r#"input.framing must be a string set to "line", "nul" or "syslen""#)
        })
        .to_owned();
