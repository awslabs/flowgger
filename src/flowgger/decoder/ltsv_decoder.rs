    ...
    pub fn new(config: &Config) -> LTSVDecoder {
        let schema = match config.lookup("input.ltsv_schema") {
            ...
            Some(pairs) => {
                ...
                for (name, sdtype) in
                    pairs.as_table().expect("input.ltsv_schema must be a list of key/type pairs") {
                    let sdtype = match sdtype.as_str()
                        ...
                        "u64" => SDValueType::U64,
                        _ => {
                            panic!(format!("Unsupported type in input.ltsv_schema for name [{}]",
                                           name))
                        }
                    };
        ...
        match config.lookup("input.ltsv_suffixes") {
            None => {}
            Some(pairs) => {
                for (sdtype, suffix) in pairs.as_table()
                    ...
                    match sdtype.to_lowercase().as_ref() {
                        ...
                        _ => {
                            panic!(format!("Unsupported type in input.ltsv_suffixes for type [{}]",
                                           sdtype))
                        }
                    }
                }
...
fn parse_ts(line: &str) -> Result<f64, &'static str> {
    unix_strtime_to_unix(line).or(rfc3339_to_unix(line)).or(english_time_to_unix(line))
}
...
fn test_ltsv_suffixes() {
    ...
    assert!(pairs.iter().cloned().any(|(k, v)| if let SDValue::I64(v) = v {
                                          k == "_score_i64" && v == -1
                                      } else {
                                          false
                                      }));
    assert!(pairs.iter().cloned().any(|(k, v)| if let SDValue::F64(v) = v {
                                          k == "_mean_f64" && f64::abs(v - 0.42) < 1e-5
                                      } else {
                                          false
                                      }));
    assert!(pairs.iter().cloned().any(|(k, v)| if let SDValue::Bool(v) = v {
                                          k == "_done_bool" && v == true
                                      } else {
                                          false
                                      }));
