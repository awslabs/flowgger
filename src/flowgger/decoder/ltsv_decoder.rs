...
fn test_ltsv() {
    ...
    assert!(pairs.iter().cloned().any(|(k, v)|
        if let SDValue::I64(v) = v { k == "_score" && v == -1 } else { false }
    ));
    assert!(pairs.iter().cloned().any(|(k, v)|
        if let SDValue::F64(v) = v { k == "_mean" && f64::abs(v - 0.42) < 1e-5 } else { false }
    ));
    assert!(pairs.iter().cloned().any(|(k, v)|
        if let SDValue::Bool(v) = v { k == "_done" && v == true } else { false }
    ));
}
