...
impl KafkaOutput {
    pub fn new(config: &Config) -> KafkaOutput {
        let acks = config.lookup("output.kafka_acks").map_or(KAFKA_DEFAULT_ACKS, |x| {
            x.as_integer().expect("output.kafka_acks must be a 16-bit integer") as i16
        });
        let brokers = config.lookup("output.kafka_brokers")
        ...
        let coalesce = config.lookup("output.kafka_coalesce").map_or(KAFKA_DEFAULT_COALESCE, |x| {
