...
use openssl::dh::DH;
use openssl::ssl::{SslContext, SslStream, SslMethod, SSL_VERIFY_NONE, SSL_VERIFY_PEER,
                   SSL_VERIFY_FAIL_IF_NO_PEER_CERT, SSL_OP_NO_COMPRESSION,
...
fn config_parse(config: &Config) -> (TlsConfig, u32) {
    let threads = config.lookup("output.tls_threads").map_or(TLS_DEFAULT_THREADS, |x| {
        x.as_integer().expect("output.tls_threads must be a 32-bit integer") as u32
    });
    let connect = config.lookup("output.connect")
    ...
    let ciphers = config.lookup("output.tls_ciphers")
        .map_or(DEFAULT_CIPHERS,
    ...
    let verify_peer = config.lookup("output.tls_verify_peer").map_or(DEFAULT_VERIFY_PEER, |x| {
        x.as_bool().expect("output.tls_verify_peer must be a boolean")
    });
    ...
    let compression = config.lookup("output.tls_compression").map_or(DEFAULT_COMPRESSION, |x| {
        x.as_bool().expect("output.tls_compression must be a boolean")
    });
    let timeout = config.lookup("output.timeout").map_or(DEFAULT_TIMEOUT, |x| {
        x.as_integer().expect("output.timeout must be an integer") as u64
    });
    let async = config.lookup("output.tls_async").map_or(DEFAULT_ASYNC, |x| {
