...
fn config_parse(config: &Config) -> (TlsConfig, u32) {
    ...
    let mut connect: Vec<String> = connect
        ...
        .collect();
    let cert: Option<PathBuf> = config
        .lookup("output.tls_cert")
        .map_or(None, |x| {
            Some(PathBuf::from(x.as_str()
                                   .expect("output.tls_cert must be a path to a .pem file")))
        });
    let key: Option<PathBuf> = config
        .lookup("output.tls_key")
        .map_or(None, |x| {
            Some(PathBuf::from(x.as_str()
    ...
    let ca_file: Option<PathBuf> = config
        .lookup("output.tls_ca_file")
        .map_or(None, |x| {
            Some(PathBuf::from(x.as_str()
    ...
    let async = config
        ...
        .map_or(DEFAULT_ASYNC,
                |x| x.as_bool().expect("output.tls_async must be a boolean"));
    let recovery_delay_init = config
    ...
    let recovery_probe_time = config
    ...
    let mut connector_builder = SslConnectorBuilder::new(SslMethod::tls()).unwrap();
    {
        ...
        let mut opts = SSL_OP_CIPHER_SERVER_PREFERENCE |
